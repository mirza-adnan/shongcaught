...
// Plain-language explanations for someone with no technical background — this is what actually
// gets shown to agents and ops, not "z-score" or "voter" jargon. The evidence panel underneath
// still has the precise numbers for anyone who wants them.
const CATEGORY_PLAIN_LANGUAGE: Record<AnomalyCategory, string> = {
    velocity:
        "This account had a lot more transactions than usual in a short amount of time.",
    near_identical_amounts:
        "Several transactions for almost exactly the same amount happened close together, involving only a couple of people — that's an unusual pattern.",
    structuring:
        "Many small transactions from the same person added up to a large total. Sometimes people split up payments like this on purpose, though there can be an innocent reason too.",
    circular:
        "Money appears to be moving back and forth between the same small group of people.",
    balance_inconsistency:
        "The recorded balance doesn't quite match up with the transactions — worth double-checking the numbers.",
    other: "Something about this account's recent activity looks different from its usual pattern.",
};

const CATEGORY_PLAIN_LANGUAGE_BN: Record<AnomalyCategory, string> = {
    velocity:
        "Ei account-e shovabik shomoyer cheye onek beshi lenden hoye geche kom shomoyer moddhe.",
    near_identical_amounts:
        "Kom shomoyer moddhe prai eki poriman-er koyekti lenden hoyeche, khub kom shonkhok manusher shathe — eta oshbabhabik lagche.",
    structuring:
        "Ekjon manusher kach theke onek choto choto lenden mile boro ekta total hoye geche. Ei rokom kore kore keu keu ichchha kore o kore, tobe nirdosh karonao thakte pare.",
    circular:
        "Taka mone hocche eki choto ekta doler moddhe ghure ghure jacche.",
    balance_inconsistency:
        "Recorded balance ar lenden thik mile na — number ta abar check kore dekha dorkar.",
    other: "Ei account-er recent activity shovabik theke kichuta alada mone hocche.",
};
...
function pickMajorityCategory(
    items: { category: AnomalyCategory; confidence: number }[],
): AnomalyCategory {
    const stats = new Map<
        AnomalyCategory,
        { count: number; confidenceSum: number }
    >();
...
async function velocityVoter(
    ...
    const historyRows = await getRecentTransactions(
        agentId,
        provider,
        HISTORY_HOURS,
        referenceTime,
    ...
    const meanPerWindow =
        baselineWindows > 0 ? baselineCount / baselineWindows : 0;
...
function nearIdenticalVoter(rows: TxRow[]): VoterResult {
    ...
    for (const row of rows) {
        ...
        const bucket = buckets.find(
            (b) =>
                Math.abs(b.amount - amount) / Math.max(b.amount, amount) <=
                epsilon,
        );
    ...
    return {
        ...
        confidence: fired
            ? Math.min(0.9, 0.5 + candidate.rows.length * 0.05)
            : 0.2,
        rationale: fired
        ...
        evidence: {
            clusterAmount: candidate.amount,
            clusterSize: candidate.rows.length,
            distinctParties,
        },
    };
}
...
function balanceReconciliationVoter(rows: TxRow[]): VoterResult {
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        const expected =
            Number(prev.providerBalanceAfter) +
            deltaFor(cur.type, Number(cur.amount));
        if (Math.abs(expected - Number(cur.providerBalanceAfter)) > 1)
            mismatches++;
    }
...
async function llmVoter(prompt: string): Promise<VoterResult> {
    try {
        ...
        return {
            ...
            fired: false,
            confidence: 0,
            rationale: "LLM voter unavailable for this check.",
        };
    }
}
...
async function analyzeAgentProvider(
    ...
    const category = pickMajorityCategory(
        firing.map((v) => ({
            category: v.category!,
    ...
    const confidence =
        (firing.length / voters.length) *
        (firing.reduce((s, v) => s + v.confidence, 0) / firing.length);

    const llmVote = voters.find((v) => v.voter === "llm");
    const llmReason = llmVote?.fired
        ? ` Our AI assistant noticed the same thing: ${llmVote.rationale}`
        : "";

    const scenarioTag =
        recentRows.find((r) => r.scenarioTag)?.scenarioTag ?? null;

    const providerLabel = PROVIDER_LABELS[provider];

    const description =
        `${CATEGORY_PLAIN_LANGUAGE[category]} We noticed this on ${agent.name}'s ${providerLabel} account ` +
        `— ${firing.length} out of ${voters.length} separate checks agreed something looks off. This is ` +
        `not proof that anything is wrong, just worth a quick look.${llmReason}`;

    const banglishSummary =
        `${CATEGORY_PLAIN_LANGUAGE_BN[category]} Eta ${agent.name}-er ${providerLabel} account-e dhora ` +
        `poreche (${firing.length}/${voters.length} check-e dhora poreche, ${Math.round(confidence * 100)}% ` +
        `nishchoyota). Eta kono proman na je kichu bhul hoyeche — shudhu ektu dekhe newa dorkar.`;
    ...
    return upsertAlert({
        ...
        provider,
        severity,
        title: `Worth a look: ${agent.name}'s ${providerLabel} activity`,
        description,
        banglishSummary,
        evidence: {
            windowHours: WINDOW_HOURS,
            transactionCount: recentRows.length,
        },
        confidence,
        category,
        votes: voters,
...
export async function analyzeAnomaly(agentId: string) {
    ...
    const results = await Promise.all(
        PROVIDERS.map((provider) =>
            analyzeAgentProvider(agent, provider, referenceTime),
        ),
    );
